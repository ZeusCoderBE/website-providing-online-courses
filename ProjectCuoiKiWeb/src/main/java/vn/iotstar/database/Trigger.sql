...
BEGIN
	...
	WHERE hoc.TrangThai = 'Done' and hoc.MaNguoiDung = @manguoidung and kh.MaKhoaHoc = (SELECT Distinct MaKhoaHoc 
						  ...
						  WHERE bh.MaBaiHoc = @mabaihoc)
	--Tìm tiến độ cũ
	declare @tiendocu real
	select @tiendocu=TienDo From DangKy
	where MaKhoaHoc=(SELECT Distinct MaKhoaHoc 
	                      FROM BAIHOC AS bh
						  WHERE bh.MaBaiHoc = @mabaihoc)
...
BEGIN
   ...
   declare @mand int
   declare @mabh int
   declare cursorBaiHoc Cursor for
   SELECT MaNguoiDung, MaBaiHoc
   FROM @TempTable
   WHERE MaNguoiDung IS NOT NULL
   Open cursorBaiHoc
   fetch next from cursorBaiHoc into @mand,@mabh
   while @@FETCH_STATUS=0
   begin
	insert into HOC(MaNguoiDung,MaBaiHoc)
	values(@mand,@mabh)
	fetch next from cursorBaiHoc
	into @mand,@mabh
   end
   deallocate cursorBaiHoc
END
Go
